...
class ModelTrainer:
    ...
    def get_model_object_and_report(
        self, train: NDArray, test: NDArray
        ...
        try:
            ...
            logging.info(f"Test Accuracy: {test_accuracy:.4f}, F1 Score: {f1:.4f}")
            ...
            error_message = f"Error in get_model_object_and_report: {str(e)}"
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        ...
        try:
            ...
            if train_accuracy < self.model_trainer_config.expected_accuracy:
                logging.info(f"Model accuracy {train_accuracy} is below the expected threshold {self.model_trainer_config.expected_accuracy}")
                raise MyException("No model found with score above the base score", sys)
            ...
            error_message = f"Error in initiate_model_trainer: {str(e)}"
            logging.error(error_message)
            raise MyException(error_message, sys) from e
